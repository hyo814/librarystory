...
<HTML>
<HEAD><TITLE>library-return</TITLE></HEAD>
<BODY>
	<H1>반납</H1>
	<TABLE border=1>
    <TR>
    ...
    <c:forEach var="cnt" begin="0" end="${LISTSIZE- 1}"> 
        <TR>
            ...
            <TD>
            <c:if test="${USERLIBRARY_LIST.lend[cnt]==1}">Y</c:if>
            <c:if test="${USERLIBRARY_LIST.lend[cnt]==0}">N</c:if></TD>
            <TD>${USERLIBRARY_LIST.number[cnt]}</TD>
            <td>
            <a href="lib-returnClick?id=${USERLIBRARY_LIST.id[cnt]}">반납</a></td>
        </TR>
    </c:forEach> 
</TABLE>
</BODY>
